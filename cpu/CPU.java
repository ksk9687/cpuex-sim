...
public class CPU {
	...
	public int parse(Map<String, Integer> labels, String s, int len, int offset, boolean canNeg) {
		int i;
		if (labels != null && labels.containsKey(s)) i = labels.get(s);
		else if (s.startsWith("0x")) i = Integer.parseInt(s.substring(2), 16);
		else if (s.startsWith("0b")) i = Integer.parseInt(s.substring(2), 2);
		else i = Integer.parseInt(s) - offset;
		if (i < 0) {
			if (!canNeg || i <= (-1 ^ 1 << (len - 1))) throw new NumberFormatException();
		} else {
	...
	public int parseRegister(String s) {
		...
		return parse(null, s.substring(1), 5, 0, false);
	}
		...
		for (int i = 0; i < n; i++) {
			...
			try {
				if (ss[0].equals("add")) {
					...
					binary[i] = 1 << 26 | parseRegister(ss[j++]) << 21 | parseRegister(ss[j++]) << 16 | parse(labels, ss[j++], 16, 0, true);
					...
					binary[i] = 3 << 26 | parseRegister(ss[j++]) << 21 | parseRegister(ss[j++]) << 16 | parse(null, ss[j++], 5, 0, false);
				} else if (ss[0].equals("sll")) {
					binary[i] = 4 << 26 | parseRegister(ss[j++]) << 21 | parseRegister(ss[j++]) << 16 | parse(null, ss[j++], 5, 0, false);
					...
					binary[i] = 9 << 26 | parseRegister(ss[j++]) << 21 | parseRegister(ss[j++]) << 16 | parse(labels, ss[j++], 16, 0, true);
				} else if (ss[0].equals("li")) {
					binary[i] = 10 << 26 | parseRegister(ss[j++]) << 16 | parse(labels, ss[j++], 16, 0, false);
				} else if (ss[0].equals("store")) {
					binary[i] = 11 << 26 | parseRegister(ss[j++]) << 21 | parseRegister(ss[j++]) << 16 | parse(labels, ss[j++], 16, 0, true);
					...
					binary[i] = 13 << 26 | parseRegister(ss[j++]) << 21 | parse(null, ss[j++], 3, 0, false) << 16 | parse(labels, ss[j++], 16, i, true);
				} else if (ss[0].equals("jal")) {
					binary[i] = 14 << 26 | parse(labels, ss[j++], 26, 0, false);
					...
					throw new ParseException(String.format("%d: 不正な命令 \"%s\"", lines[i], ss[0]));
				}
				if (j != ss.length) throw new ArrayIndexOutOfBoundsException();
			} catch (ArrayIndexOutOfBoundsException e) {
				throw new ParseException(String.format("%d: 不正なオペランド数", lines[i]));
			} catch (NumberFormatException e) {
				throw new ParseException(String.format("%d: 不正なオペランド \"%s\"", lines[i], ss[j - 1]));
			}
		}
		return binary;
	}
